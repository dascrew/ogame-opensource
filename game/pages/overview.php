...
<?php
    if ($now < $uni['news_until']) {        // Show the news?
        ...
        if (!empty($GlobalUni['ext_board'])) {
            $combox_url = $GlobalUni['ext_board'];
        } elseif (!empty($GlobalUni['ext_discord'])) {
            $combox_url = $GlobalUni['ext_discord'];
        }
        ?>
...
if ($aktplanet['type'] == PTYP_MOON) {
    $name = va(loca('OVERVIEW_MOON'), $aktplanet['name'], $aktplanet['g'], $aktplanet['s'], $aktplanet['p']);
} else {
    $name = va(loca('OVERVIEW_PLANET'), $aktplanet['name']);
}
...
for ($i = 0; $i < $num; $i++) {
    ...
    if ($planet['type'] == PTYP_MOON || $planet['planet_id'] == $aktplanet['planet_id']) {
        $num--;
        $i--;
        continue;
    }
