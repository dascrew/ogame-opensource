...
<?php
        if ($myrank['rights'] & ARANK_ONLINE) {
            echo "    <th><a href=\"index.php?page=allianzen&session=$session&a=4&sort1=5&sort2=" . ($sort2 ^ 1) . "\">Online</a></th></tr>\n";
        }
    if (($myrank['rights'] & ARANK_ONLINE) == 0 && $sort1 == 5) {
        $sort1 = 0;
    }
    ?>
<?php
    ...
    for ($i = 0; $i < $members; $i++) {
        ...
        if ($myrank['rights'] & ARANK_ONLINE) {
            ...
            if ($min < 15) {
                echo '    <th><font color=lime>' . loca('ALLY_MEMBERS_YES') . '</font></th>';
            } elseif ($min < 60) {
                echo "    <th><font color=yellow>$min min</font></th>";
            } else {
                echo '    <th><font color=red>' . loca('ALLY_MEMBERS_NO') . '</font></th>';
            }
        }
        echo "</tr>\n";
    }
    ...
    if (method() === 'GET' && $_GET['a'] == 13 && $selected_user) {        // Kick member
        ...
        while ($rows--) {
            ...
            SendMessage(
                ...
                va(loca_lang('ALLY_MSG_KICK_TEXT', $user['lang']), $leaver['oname']),
                MTYP_ALLY
            );
        }
        ...
        SendMessage(
            ...
            va(loca_lang('ALLY_MSG_YOU_KICKED', $leaver['lang']), $GlobalUser['oname'], $ally['tag']),
            MTYP_ALLY
        );
    }
