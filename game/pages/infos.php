...
function rapid($gid)
{
    ...
    $res = '';
    for ($n = 202; $n <= 215; $n++) {
        if (key_exists($n, $RapidFire[$gid]) && $RapidFire[$gid][$n] > 1) {
            $res .= rapidOut($n, $RapidFire[$gid][$n]);
        }
    }
    for ($n = 401; $n <= 408; $n++) {
        if (key_exists($n, $RapidFire[$gid]) && $RapidFire[$gid][$n] > 1) {
    ...
    for ($n = 202; $n <= 215; $n++) {
        if (key_exists($gid, $RapidFire[$n]) && $RapidFire[$n][$gid] > 1) {
            $res .= rapidIn($n, $RapidFire[$n][$gid]);
        }
    }
    for ($n = 401; $n <= 408; $n++) {
        if (key_exists($gid, $RapidFire[$n]) && $RapidFire[$n][$gid] > 1) {
...
if (isFleet($gid)) {    // Fleet
...
} elseif (isDefenseNoRak($gid)) {    // Defense.
    echo "<!-- begin fleet or defense information -->\n";
    ...
    echo '<tr><th>' . loca('INFO_ATTACK') . '</th><th>' . nicenum($UnitParam[$gid][2]) . "</th></tr>\n";
    echo "</th></tr></table>\n";
} elseif (isResearch($gid)) {    // Research.
    ...
    if ($gid == GID_B_METAL_MINE) {    // Metal mine
        ...
        for ($i = $level; $i < $level + 15; $i++) {
            ...
            if ($i == $aktplanet['b' . $gid]) {
                echo "<tr> <th> <font color=#FF0000>$i</font></th> ";
            } else {
                echo "<tr> <th> $i</th> ";
            }
    ...
    } elseif ($gid == GID_B_CRYS_MINE) {    // Crystal mine
        echo "<tr><th><p><center><table border=1 ><tr><td class='c'>" . loca('INFO_LEVEL') . "</td><td class='c'>" . loca('INFO_PROD') . "</td><td class='c'>" . loca('INFO_DIFF') . "</td><td class='c'>" . loca('INFO_ENERGY') . "</td><td class='c'>" . loca('INFO_DIFF') . "</td> \n";
        $level = $aktplanet['b' . $gid] - 2;
        if ($level <= 0) {
            $level = 1;
        ...
        echo '</table></center></tr></th>';
    } elseif ($gid == GID_B_DEUT_SYNTH) {    // Deuterium synthesizer
        echo "<tr><th><p><center><table border=1 ><tr><td class='c'>" . loca('INFO_LEVEL') . "</td><td class='c'>" . loca('INFO_PROD') . "</td><td class='c'>" . loca('INFO_DIFF') . "</td><td class='c'>" . loca('INFO_ENERGY') . "</td><td class='c'>" . loca('INFO_DIFF') . "</td> \n";
        $level = $aktplanet['b' . $gid] - 2;
        if ($level <= 0) {
            $level = 1;
        ...
        echo '</table></center></tr></th>';
    } elseif ($gid == GID_B_SOLAR) {    // Solar Plant
        ...
        echo '</table></center></tr></th>';
    } elseif ($gid == GID_B_FUSION) {    // Fusion Reactor
        ...
        echo '</table></center></tr></th>';
    } elseif ($gid == GID_B_METAL_STOR || $gid == GID_B_CRYS_STOR || $gid == GID_B_DEUT_STOR) {     // Storages
        ...
        echo '</table>';
    } elseif ($gid == GID_B_ALLY_DEPOT) {                                    // Alliance Depot
...
<?php
    } elseif ($gid == GID_B_MISS_SILO && $aktplanet['b' . GID_B_MISS_SILO] > 0) {        // Missile Silo
...
<?php
    ...
    } elseif ($gid == GID_B_PHALANX) {        // Sensor Phalanx
        ?>
...
<?php
        ...
        for ($i = $level; $i < $aktplanet['b' . $gid] + 5; $i++) {
            $radius = $i * $i - 1;
            if ($i == $aktplanet['b' . $gid]) {
                echo "<tr><th align=center >&nbsp;<FONT color=FF0000>$i</FONT></th><th align=center >&nbsp;$radius&nbsp;</th></tr>";
            } else {
                echo "<tr><th align=center >&nbsp;<FONT color=FFFFFF>$i</FONT></th><th align=center >&nbsp;$radius&nbsp;</th></tr>";
            }
        }
        ?>
</center></table></tr></th></table> 
<?php
    } elseif ($gid == GID_B_JUMP_GATE && $aktplanet['b' . GID_B_JUMP_GATE] > 0) {        // Jump Gate
...
<?php
            ...
            while ($rows--) {
                ...
                if ($planet['planet_id'] == $aktplanet['planet_id']) {
                    continue;
                }    // current moon
                if ($planet['b43'] == 0) {
                    continue;
                }    // no jump gate
                if ($planet['type'] != PTYP_MOON || $now < $planet['gate_until']) {
                    continue;
                }
...
<?php
    ...
    if ($gid < 200 && $aktplanet['b' . $gid] && !($gid == GID_B_TERRAFORMER || $gid == GID_B_LUNAR_BASE || $gid == GID_B_MISS_SILO)) {
        ...
        if ($m) {
            echo loca('INFO_DEMOLISH_M') . '<b>' . nicenum($m) . '</b> ';
        }
        if ($k) {
            echo loca('INFO_DEMOLISH_K') . '<b>' . nicenum($k) . '</b> ';
        }
        if ($d) {
            echo loca('INFO_DEMOLISH_D') . '<b>' . nicenum($d) . '</b> ';
        }
    ...
    if ($gid == GID_B_MISS_SILO && $aktplanet['b' . $gid]) {    // Missile Silo
        ...
        if ($raknum == 0) {
            ...
            echo '<tr><td class=c align=center>' . loca('INFO_DEMOLISH_DEFENSE') . '</a></td></tr>';
        }
        $res = BuildPrice($gid, $aktplanet['b' . $gid] - 1);
        $m = $res['m'];
        $k = $res['k'];
