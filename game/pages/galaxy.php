...
if (method() === 'POST' && isset($_POST['aktion'])) {
    ...
    if ($target == null) {
        $GalaxyError = loca('GALAXY_RAK_NO_TARGET');
    }

    if (!in_array($type, $defmap_norak)) {
        $type = 0;
    }

    if ($GalaxyError === '') {    // Check the permitted parameters
        if ($amount == 0) {
            $GalaxyError = loca('GALAXY_RAK_NO_ROCKETS');
        }
        if ($amount > $aktplanet['d' . GID_D_IPM]) {
            $GalaxyError = loca('GALAXY_RAK_NOT_ENOUGH');
        }
        if ($dist > $ipm_radius) {
            $GalaxyError = loca('GALAXY_RAK_WEAK_DRIVE');
        }
    }

    if ($GalaxyError === '') {        // Check player modes
        if ($GlobalUser['vacation']) {
            $GalaxyError = loca('GALAXY_RAK_VACATION_SELF');
        } elseif ($target_user['vacation']) {
            $GalaxyError = loca('GALAXY_RAK_VACATION_OTHER');
        } elseif ($target['owner_id'] == $GlobalUser['player_id']) {
            $GalaxyError = loca('GALAXY_RAK_SELF');
        } elseif (IsPlayerNewbie($target_user['player_id']) || IsPlayerStrong($target_user['player_id'])) {
            $GalaxyError = loca('GALAXY_RAK_NOOB');
        }
    }
...
} elseif (key_exists('galaxy', $_GET)) {
    $coord_g = intval($_GET['galaxy']);
} elseif (key_exists('p1', $_GET)) {
    $coord_g = intval($_GET['p1']);
} else {
    $coord_g = $aktplanet['g'];
}
if (key_exists('session', $_POST)) {
    $coord_s = intval($_POST['system']);
} elseif (key_exists('system', $_GET)) {
    $coord_s = intval($_GET['system']);
} elseif (key_exists('p2', $_GET)) {
    $coord_s = intval($_GET['p2']);
} else {
    $coord_s = $aktplanet['s'];
}
if (key_exists('session', $_POST)) {
    $coord_p = 0;
} elseif (key_exists('position', $_GET)) {
    $coord_p = intval($_GET['position']);
} elseif (key_exists('p3', $_GET)) {
    $coord_p = intval($_GET['p3']);
} else {
    $coord_p = $aktplanet['p'];
}
...
if ($coord_g > $GlobalUni['galaxies']) {
    $coord_g = $GlobalUni['galaxies'];
}

if ($coord_s < 1) {
    $coord_s = 1;
}
if ($coord_s > $GlobalUni['systems']) {
    $coord_s = $GlobalUni['systems'];
}
    ...
    while ($num--) {
        ...
        if ($planet['type'] == PTYP_PLANET) {
            ...
            if ($own) {
                ...
                if ($phalanx) {
                    echo '<a href=# onclick=fenster(&#039;index.php?page=phalanx&session=' . $_GET['session'] . '&scanid=' . $planet['owner_id'] . '&spid=' . $planet['planet_id'] . '&#039;) >' . loca('GALAXY_FLEET_PHALANX') . '</a><br />';
                }
                if ($show_ipm_button) {
                    echo "<a href=index.php?page=galaxy&no_header=1&session=$session&mode=1&p1=" . $planet['g'] . '&p2=' . $planet['s'] . '&p3=' . $planet['p'] . '&pdd=' . $planet['planet_id'] . '&zp=' . $planet['owner_id'] . ' >' . loca('GALAXY_FLEET_RAK') . '</a><br />';
                }
            ...
            if ($GlobalUser['admin'] >= 2) {
                echo "<a href=index.php?page=admin&session=$session&mode=Planets&cp=" . $planet['planet_id'] . '>' . loca('GALAXY_PLANET_ADMIN') . '</a><br />';
            }
        ...
        if (!$own) {
            ...
            if ($moon_id && $moon['lastakt'] > $planet['lastakt']) {
                $activity = $moon['lastakt'];
            }
            if ($activity > $ago15) {
                $akt = '&nbsp;(*)';
            } elseif ($activity > $ago60) {
                $akt = '&nbsp;(' . floor(($now - $activity) / 60) . ' min)';
            }
        }
        if ($planet['type'] == PTYP_DEST_PLANET) {
            $planet_name = loca('PLANET_DESTROYED') . $akt;
        } elseif ($planet['type'] == PTYP_ABANDONED) {
            $planet_name = loca('PLANET_ABANDONED') . $akt;
            $phalanx = false;
        } else {
            $planet_name = $planet['name'] . $akt;
        }
        if ($phalanx) {
            $planet_name = "<a href='#' onclick=fenster('index.php?page=phalanx&session=$session&scanid=" . $planet['owner_id'] . '&spid=' . $planet['planet_id'] . "',\"Bericht_Phalanx\") title=\"" . loca('GALAXY_FLEET_PHALANX') . '">' . $planet_name . '</a>';
        }
        ...
        if ($moon_id) {
            if ($moon['type'] == PTYP_MOON) {
                ...
                if ($own) {
                    ...
                    if ($show_ipm_button) {
                        echo "<a href=index.php?page=galaxy&no_header=1&session=$session&mode=1&p1=" . $moon['g'] . '&p2=' . $moon['s'] . '&p3=' . $moon['p'] . '&pdd=' . $moon['planet_id'] . '&zp=' . $moon['owner_id'] . ' >' . loca('GALAXY_FLEET_RAK') . '</a><br />';
                    }
                    echo '<a href=index.php?page=flotten1&session=' . $_GET['session'] . '&galaxy=' . $moon['g'] . '&system=' . $moon['s'] . '&planet=' . $moon['p'] . '&planettype=3&target_mission=3 >' . loca('GALAXY_FLEET_TRANSPORT') . '</a><br />';
                ...
                if ($GlobalUser['admin'] >= 2) {
                    echo "<a href=index.php?page=admin&session=$session&mode=Planets&cp=" . $moon['planet_id'] . '>' . loca('GALAXY_PLANET_ADMIN') . '</a><br />';
                }
                ...
                echo '<div style="border: 2pt solid #FF0000;"><img src="' . GetPlanetSmallImage(UserSkin(), $moon) . '" alt="' . va(loca('GALAXY_MOON_TITLE_SIZE'), $moon['diameter']) . "\" height=\"22\" width=\"22\" onmouseover=\"return overlib('<font color=white><b>" . loca('MOON_DESTROYED') . "</b></font>', WIDTH, 75);\" onmouseout=\"return nd();\"/></div>\n";
            }
        }
        echo "</th>\n";
...
<?php
        ...
        if (!($planet['type'] == PTYP_DEST_PLANET || $planet['type'] == PTYP_ABANDONED)) {
            ...
            if ($GlobalUser['admin'] >= 2) {
                echo "<tr><td><a href=index.php?page=admin&session=$session&mode=Users&player_id=" . $user['player_id'] . '>' . loca('GALAXY_USER_ADMIN') . '</a></td></tr>';
            }
            echo '</table>';
            ...
            if (IsPlayerNewbie($user['player_id'])) {
            ...
            } elseif (IsPlayerStrong($user['player_id'])) {
                $pstat = 'strong';
                ...
                if ($user['banned']) {
                    if (mb_strlen($stat, 'UTF-8')) {
                        $stat .= ' ';
                    } $stat .= "<a href='index.php?page=pranger&session=" . $_GET['session'] . "'><span class='banned'>" . loca('GALAXY_LEGEND_BANNED') . '</span></a>';
                    $pstat = 'banned';
                }
                if ($user['lastclick'] <= $week4) {
                    if (mb_strlen($stat, 'UTF-8')) {
                        $stat .= ' ';
                    } $stat .= "<span class='longinactive'>" . loca('GALAXY_LEGEND_INACTIVE28') . '</span>';
                    if ($pstat !== 'banned') {
                        $pstat = 'longinactive';
                    }
                }
                if ($user['vacation']) {
                    if (mb_strlen($stat, 'UTF-8')) {
                        $stat .= ' ';
                    } $stat .= "<span class='vacation'>" . loca('GALAXY_LEGEND_VACATION') . '</span>';
                    $pstat = 'vacation';
                }
            }
