...
if (key_exists('gesendet', $_GET)) {
    if ($_GET['gesendet'] == 1) {
        ...
        $betreff = $subj;
        if ($subj === '') {
            $write_error = '<center><font color=#FF0000>' . loca('WRITE_MSG_ERROR_NO_SUBJ') . "</font><br/><br/></center>\n";
        } elseif ($text === '') {
            $write_error .= '<center><font color=#FF0000>' . loca('WRITE_MSG_ERROR_NO_BODY') . "</font><br/><br/></center>\n";
        } else {
            if ($user['useskin']) {
                $skin = $user['skin'];
            } else {
