...
if (!key_exists('resource1', $_POST)) {
    $_POST['resource1'] = 0;
}
if (!key_exists('resource2', $_POST)) {
    $_POST['resource2'] = 0;
}
if (!key_exists('resource3', $_POST)) {
    $_POST['resource3'] = 0;
}
...
foreach ($fleetmap as $i => $gid) {
    if (key_exists("ship$gid", $_POST)) {
        $fleet[$gid] = min($aktplanet["f$gid"], intval($_POST["ship$gid"]));
    } else {
        $fleet[$gid] = 0;
    }
}
...
if ($GlobalUni['freeze']) {
    FleetError(loca('FLEET_ERR_FREEZE'));
}
...
if ($origin_user['vacation']) {
    FleetError(loca('FLEET_ERR_VACATION_SELF'));
}
if ($target_user['vacation'] && $order != FTYP_RECYCLE) {
    FleetError(loca('FLEET_ERR_VACATION_OTHER'));
}
if ($nowfleet >= $maxfleet) {
    FleetError(loca('FLEET_ERR_MAX_FLEET'));
}
...
if ($order == FTYP_EXPEDITION) {
    if (key_exists('expeditiontime', $_POST)) {
        ...
        if ($hold_time > $GlobalUser['r124']) {
            $hold_time = $GlobalUser['r124'];
        }
        if ($hold_time < 1) {
            $hold_time = 1;
        }
...
if ($origin['d'] < ($cons['fleet'] + $cons['probes'])) {
    FleetError(loca('FLEET_ERR_FUEL'));
} elseif ($space < 0) {
    FleetError(loca('FLEET_ERR_CARGO'));
}
...
if ($numships <= 0) {
    FleetError(loca('FLEET_ERR_NO_SHIPS'));
}
...
switch ($order) {
    case FTYP_ATTACK:        // Attack
        if ($target == null) {
            ...
            ($origin_user['ally_id'] == $target_user['ally_id'] && $origin_user['ally_id'] > 0) ||
             IsBuddy($origin_user['player_id'], $target_user['player_id'])
        )) {
            $BlockAttack = 0;
        }

        if (IsPlayerNewbie($target['owner_id']) || IsPlayerStrong($target['owner_id'])) {
            FleetError(loca('FLEET_ERR_NOOB'));
        } elseif ($target['owner_id'] == $origin['owner_id']) {
            FleetError(loca('FLEET_ERR_OWN_PLANET'));
        } elseif ($BlockAttack) {
            FleetError(loca('FLEET_ERR_ATTACK_BAN_UNI'));
        } elseif ($GlobalUser['noattack']) {
            FleetError(va(loca('FLEET_ERR_ATTACK_BAN_PLAYER'), date('d.m.Y H:i:s', $GlobalUser['noattack_util'])));
        }
        break;
    ...
    case FTYP_ACS_ATTACK:        // ACS Attack
        ...
        if (! IsPlayerInUnion($GlobalUser['player_id'], $union) || $union == null) {
            FleetError(loca('FLEET_ERR_ACS_OTHER'));
        } elseif ($target['owner_id'] == $origin['owner_id']) {
            FleetError(loca('FLEET_ERR_OWN_PLANET'));
        } elseif (IsPlayerNewbie($target['owner_id']) || IsPlayerStrong($target['owner_id'])) {
            FleetError(loca('FLEET_ERR_NOOB'));
        } elseif ($flighttime > $acs_flighttime * 1.3) {
            FleetError(loca('FLEET_ERR_ACS_SLOW'));
        } elseif ($BlockAttack) {
            FleetError(loca('FLEET_ERR_ATTACK_BAN_UNI'));
        } elseif ($GlobalUser['noattack']) {
            FleetError(va(loca('FLEET_ERR_ATTACK_BAN_PLAYER'), date('d.m.Y H:i:s', $GlobalUser['noattack_util'])));
        } elseif ($acs_fleets >= $GlobalUni['acs'] * $GlobalUni['acs']) {
            FleetError(va(loca('FLEET_ERR_ACS_LIMIT'), $GlobalUni['acs'] * $GlobalUni['acs']));
        }
        break;
    ...
    case FTYP_DEPLOY:        // Deploy
        if ($target['owner_id'] !== $GlobalUser['player_id']) {
            FleetError(loca('FLEET_ERR_DEPLOY_OTHER'));
        }
        break;
    ...
    case FTYP_ACS_HOLD:        // ACS Hold
        ...
        if (GetHoldingFleetsCount($target['planet_id']) >= $maxhold_fleets) {
            FleetError(va(loca('FLEET_ERR_HOLD_FLEET_LIMIT'), $maxhold_fleets));
        }
        if (! CanStandHold($target['planet_id'], $origin['owner_id'], $maxhold_users)) {
            FleetError(va(loca('FLEET_ERR_HOLD_PLAYER_LIMIT'), $maxhold_users));
        }
        if (! (($origin_user['ally_id'] == $target_user['ally_id'] && $origin_user['ally_id'] > 0) || IsBuddy($origin_user['player_id'], $target_user['player_id']))) {
            FleetError(loca('FLEET_ERR_HOLD_ALLY'));
        }
        break;
    ...
    case FTYP_SPY:        // Espionage
        ...
        if ($target['owner_id'] == $origin['owner_id']) {
            FleetError(loca('FLEET_ERR_SPY_OWN'));
        } elseif (IsPlayerNewbie($target['owner_id']) || IsPlayerStrong($target['owner_id'])) {
            FleetError(loca('FLEET_ERR_SPY_NOOB'));
        } elseif ($fleet[GID_F_PROBE] == 0) {
            FleetError(loca('FLEET_ERR_SPY_REQUIRED'));
        } elseif ($BlockAttack) {
            FleetError(loca('FLEET_ERR_ATTACK_BAN_UNI'));
        } elseif ($GlobalUser['noattack']) {
            FleetError(va(loca('FLEET_ERR_ATTACK_BAN_PLAYER'), date('d.m.Y H:i:s', $GlobalUser['noattack_util'])));
    ...
    case FTYP_COLONIZE:        // Colonize
        if ($fleet[GID_F_COLON] == 0) {
            FleetError(loca('FLEET_ERR_COLONY_REQUIRED'));
        } elseif (HasPlanet(intval($_POST['galaxy']), intval($_POST['system']), intval($_POST['planet']))) {
            FleetError(loca('FLEET_ERR_COLONY_EXISTS'));
        } else {
    ...
    case FTYP_RECYCLE:        // Recycle
        if ($fleet[GID_F_RECYCLER] == 0) {
            FleetError(loca('FLEET_ERR_RECYCLE_REQUIRED'));
        } elseif ($target['type'] != PTYP_DF) {
            FleetError(loca('FLEET_ERR_RECYCLE_DF'));
        }
        break;
    ...
    case FTYP_DESTROY:        // Destroy (moon)
        ...
        if ($fleet[GID_F_DEATHSTAR] == 0) {
            FleetError(loca('FLEET_ERR_DESTROY_REQUIRED'));
        } elseif ($target['type'] != PTYP_MOON) {
            FleetError(loca('FLEET_ERR_DESTROY_MOON'));
        } elseif ($BlockAttack) {
            FleetError(loca('FLEET_ERR_ATTACK_BAN_UNI'));
        } elseif ($GlobalUser['noattack']) {
            FleetError(va(loca('FLEET_ERR_ATTACK_BAN_PLAYER'), date('d.m.Y H:i:s', $GlobalUser['noattack_util'])));
    ...
    case FTYP_EXPEDITION:       // Expedition
        ...
        if ($expnum >= $maxexp) {
            FleetError(loca('FLEET_ERR_EXP_LIMIT'));
        } elseif ($manned == 0) {
            FleetError(loca('FLEET_ERR_EXP_REQUIRED'));
        } elseif (intval($_POST['planet']) != 16) {
            FleetError(loca('FLEET_ERR_EXP_INVALID'));
        } else {
