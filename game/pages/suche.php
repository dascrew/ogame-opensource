...
{
    if (key_exists('type', $_POST) && $_POST['type'] === $opt) {
        return 'selected';
    } else {
        return '';
    }
...
if (method() === 'POST') {
    ...
    if ($text_len >= 2) {
        ...
        if ($_POST['type'] === 'playername') {
            $query = 'SELECT * FROM ' . $db_prefix . "users WHERE oname LIKE '%" . $searchtext . "%' LIMIT " . ($SEARCH_LIMIT + 1);
        } elseif ($_POST['type'] === 'planetname') {
            $query = 'SELECT * FROM ' . $db_prefix . "planets WHERE name LIKE '%" . $searchtext . "%' LIMIT " . ($SEARCH_LIMIT + 1);
        } elseif ($_POST['type'] === 'allytag') {
            $query = 'SELECT * FROM ' . $db_prefix . "ally WHERE tag LIKE '%" . $searchtext . "%' LIMIT " . ($SEARCH_LIMIT + 1);
        } elseif ($_POST['type'] === 'allyname') {
            $query = 'SELECT * FROM ' . $db_prefix . "ally WHERE name LIKE '%" . $searchtext . "%' LIMIT " . ($SEARCH_LIMIT + 1);
        }
    }
    ...
    if ($result) {
        ...
        while ($rows--) {
            if ($_POST['type'] === 'playername') {
                ...
                } elseif ($user['ally_id'] == $GlobalUser['ally_id'] && $user['ally_id'] != 0) {
