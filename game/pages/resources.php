...
if (method() === 'POST' && !$GlobalUser['vacation']) {
    ...
    if ($last1 > 100 || $last2 > 100 || $last3 > 100 ||
         $last4 > 100 || $last12 > 100 || $last212 > 100) {
        Error('resources: Попытка установки производительности больше 100%');
    }
...
{
    if ($num > 0) {
        return '<font color="#00ff00">' . nicenum2($num) . '</font>';
    } else {
        return '<font color="#ff0000">' . nicenum2($num) . '</font>';
    }
}
...
while ($rows--) {
    $pl = dbarray($result);
    if ($pl['type'] != PTYP_MOON) {
        $count++;
    }
}
if ($count > 9) {
    echo '<br><font color=#ff000>' . loca('RES_INFO') . '</font>';
}
