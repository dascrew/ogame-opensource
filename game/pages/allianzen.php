...
if ($GlobalUser['ally_id'] == 0 && key_exists('a', $_GET)) {
    ...
    $weiter = 0;
    if (key_exists('weiter', $_GET) && $_GET['weiter'] == 1) {
        $weiter = 1;
    }
    ...
    if ($_GET['a'] == 1 && $weiter == 1) {    // Found an alliance.
        ...
        if (mb_strlen($_POST['tag'], 'UTF-8') < 3) {
            $AllianzenError = loca('ALLY_FOUND_ERROR_TAG');
        } elseif (mb_strlen($_POST['name'], 'UTF-8') < 3) {
            $AllianzenError = loca('ALLY_FOUND_ERROR_NAME');
        } elseif (IsAllyTagExist($_POST['tag'])) {
            $AllianzenError = va(loca('ALLY_FOUND_ERROR_EXISTS'), $_POST['tag']);
        } else {
    ...
    } elseif ($_GET['a'] == 2) {        // Search for an alliance (max 30 results)
...
if ($GlobalUser['ally_id'] == 0) {
    ...
    if ($app_id > 0) {
        ...
        } elseif (key_exists('a', $_GET) && $_GET['a'] == 2) {
            $search_text = '';
            if (key_exists('suchtext', $_POST)) {
                $search_text = $_POST['suchtext'];
            }
