  ...
  <table width="525"> 
    ...
    <tr> 
      <th> 
        ...
        <select name="who"> 
          <option value="player" <?php if ($who === 'player') {
              echo 'selected';
          }?>><?=loca('STAT_PLAYER');?></option> 
          <option value="ally" <?php if ($who === 'ally') {
              echo 'selected';
          }?>><?=loca('STAT_ALLY');?></option> 
        </select> 
          
        &nbsp;<?=loca('STAT_TYPE');?>&nbsp;
              
        <select name="type"> 
          <option value="ressources" <?php if ($type === 'ressources') {
              echo 'selected';
          } ?>><?=loca('STAT_BY_POINTS');?></option> 
          <option value="fleet" <?php if ($type === 'fleet') {
              echo 'selected';
          } ?>><?=loca('STAT_BY_FLEETS');?></option> 
          <option value="research" <?php if ($type === 'research') {
              echo 'selected';
          } ?>><?=loca('STAT_BY_RESEARCH');?></option> 
        </select> 
          
        &nbsp;<?=loca('STAT_ON_PLACE');?>        <select name="start"> 
          <option value="-1" <?php if ($start == -1) {
              echo 'selected';
          }?>><?=loca('STAT_OWN_POSITION');?></option> 
<?php
...
if ($who === 'ally') {
    ...
    if ($type === 'fleet') {
        $query = 'SELECT * FROM ' . $db_prefix . "ally WHERE place2 >= $start AND place2 < " . ($start + 99) . ' ORDER BY place2;';
    } elseif ($type === 'research') {
        $query = 'SELECT * FROM ' . $db_prefix . "ally WHERE place3 >= $start AND place3 < " . ($start + 99) . ' ORDER BY place3;';
    } else {
        $query = 'SELECT * FROM ' . $db_prefix . "ally WHERE place1 >= $start AND place1 < " . ($start + 99) . ' ORDER BY place1;';
    }
    ...
    while ($rows--) {
        ...
        if ($type === 'fleet') {
            $place = $ally['place2'];
            $diff = $ally['place2'] - $ally['oldplace2'];
            $score = $ally['score2'];
        } elseif ($type === 'research') {
            $place = $ally['place3'];
            $diff = $ally['place3'] - $ally['oldplace3'];
            $score = $ally['score3'];
        } else {
            $place = $ally['place1'];
            $diff = $ally['place1'] - $ally['oldplace1'];
            $score = floor($ally['score1'] / 1000);
        }
...
<?php
        ...
        if ($diff < 0) {
            echo "      <a href='#' onmouseover='return overlib(\"<font color=lime>+" . abs($diff) . '</font><br/><font color=white>' . $date_change . "\");' onmouseout='return nd();'><font color='lime'>+</font></a> \n";
        } elseif ($diff > 0) {
            echo "      <a href='#' onmouseover='return overlib(\"<font color=red>-" . abs($diff) . '</font><br/><font color=white>' . $date_change . "\");' onmouseout='return nd();'><font color='red'>-</font></a> \n";
        } else {
            echo "      <a href='#' onmouseover='return overlib(\"<font color=87CEEB>*</font><br/><font color=white>" . $date_change . "\");' onmouseout='return nd();'><font color='87CEEB'>*</font></a> \n";
        }
        ?>    </th>
...
<?php
            if ($ally['ally_id'] == $GlobalUser['ally_id']) {
                echo "      <a href=\"#\" style='color:lime;'>\n";
            } else {
                echo '      <a href="ainfo.php?allyid=' . $ally['ally_id'] . "\" target='_ally'>      \n";
            }
        ?>
...
<?php
    ...
    if ($start <= 0) {
        if ($type === 'fleet') {
            $start = (floor($GlobalUser['place2'] / 100) * 100 + 1);
        } elseif ($type === 'research') {
            $start = (floor($GlobalUser['place3'] / 100) * 100 + 1);
        } else {
            $start = (floor($GlobalUser['place1'] / 100) * 100 + 1);
        }
    }

    if ($type === 'fleet') {
        $query = 'SELECT * FROM ' . $db_prefix . "users WHERE place2 >= $start AND place2 < " . ($start + 99) . ' ORDER BY place2;';
    } elseif ($type === 'research') {
        $query = 'SELECT * FROM ' . $db_prefix . "users WHERE place3 >= $start AND place3 < " . ($start + 99) . ' ORDER BY place3;';
    } else {
        $query = 'SELECT * FROM ' . $db_prefix . "users WHERE place1 >= $start AND place1 < " . ($start + 99) . ' ORDER BY place1;';
    }
    ...
    while ($rows--) {
        ...
        if ($type === 'fleet') {
            $place = $user['place2'];
            $diff = $user['place2'] - $user['oldplace2'];
            $score = $user['score2'];
        } elseif ($type === 'research') {
            $place = $user['place3'];
            $diff = $user['place3'] - $user['oldplace3'];
            $score = $user['score3'];
        } else {
            $place = $user['place1'];
            $diff = $user['place1'] - $user['oldplace1'];
            $score = floor($user['score1'] / 1000);
        }
