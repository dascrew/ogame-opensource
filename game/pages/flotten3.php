...
<?php
    ...
    function is_checked($mission)
    {
        if (key_exists('target_mission', $_POST)) {
            if (intval($_POST['target_mission']) == $mission) {
                return 'checked';
            }
        }
    }
...
function is_selected($union_id)
{
    if (key_exists('union2', $_POST)) {
        if (intval($_POST['union2']) == $union_id) {
            return 'selected';
        }
    }
}
...
foreach ($fleetmap_nosat as $i => $gid) {
    if (key_exists("ship$gid", $_POST)) {
        $fleet[$gid] = intval($_POST["ship$gid"]);
    } else {
        $fleet[$gid] = 0;
    }
}
...
$missions = FleetAvailableMissions(
    intval($_POST['thisgalaxy']),
    intval($_POST['thissystem']),
    intval($_POST['thisplanet']),
    intval($_POST['thisplanettype']),
    $galaxy,
    $system,
    $planet,
    intval($_POST['planettype']),
    $fleet
);
