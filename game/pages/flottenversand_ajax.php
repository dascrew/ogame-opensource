...
if (! ($order == FTYP_SPY || $order == FTYP_RECYCLE)) {
    AjaxSendError();
}    // can only be sent to espionage or recycle
if ($order == FTYP_RECYCLE && $planettype != 2) {
    AjaxSendError();
}    // recyclers can only be sent to the debris field
if ($order == FTYP_SPY && ! ($planettype == 1 || $planettype == 3)) {
    AjaxSendError();
}     // You can only spy on planets or moons
if ($galaxy < 1 || $galaxy > $GlobalUni['galaxies']) {
    AjaxSendError();
}    // wrong coordinates (Galaxy)
if ($system < 1 || $system > $GlobalUni['systems']) {
    AjaxSendError();
}    // wrong coordinates (System)
if ($planet < 1 || $planet > 15) {
    AjaxSendError();
}    // wrong coordinates (Position)
if ($GlobalUser['vacation']) {
    AjaxSendError(605);
}    // user in vacation mode
...
if ((
 ...
 IsBuddy($GlobalUser['player_id'], $target_user['player_id'])
)) {
    $BlockAttack = 0;
}
...
if ($order == FTYP_SPY) {
    ...
    if ($target_user['admin'] > 0) {
        AjaxSendError();
    }    // the administration can't be scanned.
    ...
    if (! ($GlobalUser['deact_ip'] && $target_user['deact_ip']) && !localhost($GlobalUser['ip_addr'])) {
        if ($target_user['ip_addr'] === $GlobalUser['ip_addr']) {
            AjaxSendError(616);
        }    // multialarm
    }
...
if ($aktplanet['d'] < $cons) {
    AjaxSendError(613);
}        // not enough deut to fly.
if ($cargo < $cons) {
    AjaxSendError(615);
}        // there's no room in the cargo hold for deuterium.
