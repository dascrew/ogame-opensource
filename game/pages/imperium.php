...
if (key_exists('modus', $_GET) && !$GlobalUser['vacation']) {
    if ($_GET['modus'] === 'add') {
        BuildEnque($GlobalUser, intval($_GET['planet']), intval($_GET['techid']), 0);
    } elseif ($_GET['modus'] === 'destroy') {
        BuildEnque($GlobalUser, intval($_GET['planet']), intval($_GET['techid']), 1);
    } elseif ($_GET['modus'] === 'remove') {
        BuildDeque($GlobalUser, intval($_GET['planet']), intval($_GET['listid']));
    }
}
...
if ($planettype == 1 || $planettype == 3) {
    ...
    while ($rows--) {
        ...
        if ($planettype == 1 && $planet['type'] == PTYP_MOON) {
            continue;
        }
        if ($planettype == 3 && $planet['type'] != PTYP_MOON) {
            continue;
        }
