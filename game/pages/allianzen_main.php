...
<?php
    if ($now < $ally['tag_until']) {
        echo ' (' . va(loca('ALLY_MAIN_PREV'), $ally['old_tag']) . ')';
    }
    ?>
</th></tr>
...
<?php
    if ($now < $ally['name_until']) {
        echo ' (' . va(loca('ALLY_MAIN_PREV'), $ally['old_name']) . ')';
    }
    ?>
</th></tr>
<tr><th><?=loca('ALLY_MAIN_MEMBERS');?></th><th><?=$members;?>
<?php
    if ($rank['rights'] & ARANK_R_MEMBERS) {
        echo " (<a href=\"index.php?page=allianzen&session=$session&a=4\">" . loca('ALLY_MAIN_MEMBERS_LINK') . '</a>)';
    }
    ?>
</th></tr>
<tr><th><?=loca('ALLY_MAIN_RANK');?></th><th><?=$rank['name'];?>
<?php
    if ($rank['rights'] & ARANK_W_MEMBERS) {
        echo " (<a href=\"index.php?page=allianzen&session=$session&a=5\">" . loca('ALLY_MAIN_SETTINGS_LINK') . '</a>)';
    }
    ?>
</th></tr>
<?php
