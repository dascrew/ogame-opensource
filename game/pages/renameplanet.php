...
if (method() === 'POST') {
    if ($_POST['aktion'] === loca('REN_RENAME')) {
        ...
        $aktplanet = GetPlanet($GlobalUser['aktplanet']);
    } elseif ($_POST['aktion'] === loca('REN_ABANDON_COLONY')) {
        PlanetDestroyMenu();
    } elseif ($_POST['aktion'] === loca('REN_DELETE_PLANET')) {
        // Check the password.
            ...
            if ($planet['owner_id'] == $GlobalUser['player_id']) {
                ...
                if (intval($_POST['deleteid']) == $GlobalUser['hplanetid']) {
                    ...
                    if ($RenameError === '') {
                        ...
                        if ($planet['type'] == 0) {
                            $query = 'UPDATE ' . $db_prefix . 'planets SET type = ' . PTYP_DEST_MOON . ', owner_id = ' . USER_SPACE . ", date = $now, remove = $when, lastakt = $now WHERE planet_id = " . $planet['planet_id'] . ';';
                        } else {
                            $query = 'UPDATE ' . $db_prefix . 'planets SET type = ' . PTYP_DEST_PLANET . ', owner_id = ' . USER_SPACE . ", date = $now, remove = $when, lastakt = $now WHERE planet_id = " . $planet['planet_id'] . ';';
                        }
                        dbquery($query);
