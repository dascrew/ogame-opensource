...
// Folder control
$folders = [
    'espioopen' => [ 'pm' => MTYP_SPY_REPORT, 'flag' => USER_FLAG_FOLDER_ESPIONAGE, 'title' => loca('MSG_FOLDER1') ],
    'combatopen' => [ 'pm' => MTYP_BATTLE_REPORT_LINK, 'flag' => USER_FLAG_FOLDER_COMBAT, 'title' => loca('MSG_FOLDER2') ],
    'expopen' => [ 'pm' => MTYP_EXP, 'flag' => USER_FLAG_FOLDER_EXPEDITION, 'title' => loca('MSG_FOLDER3') ],
    'allyopen' => [ 'pm' => MTYP_ALLY, 'flag' => USER_FLAG_FOLDER_ALLIANCE, 'title' => loca('MSG_FOLDER4') ],
    'useropen' => [ 'pm' => MTYP_PM, 'flag' => USER_FLAG_FOLDER_PLAYER, 'title' => loca('MSG_FOLDER5') ],
    'generalopen' => [ 'pm' => MTYP_MISC, 'flag' => USER_FLAG_FOLDER_OTHER, 'title' => loca('MSG_FOLDER6') ],
];
...
if (method() === 'POST') {
    ...
    if ($_POST['deletemessages'] === 'deleteall') {
        ...
        while ($num--) {
            $msg = dbarray($result);
            $pm = $msg['pm'];
            if ($pm == MTYP_BATTLE_REPORT_TEXT) {
                continue;
            }    // Skip the texts of battle reports.
            ...
            $msg_id = $msg['msg_id'];
            if (key_exists('sneak' . $msg_id, $_POST) && $_POST['sneak' . $msg_id] === 'on') {
                ReportMessage($player_id, $msg_id, $MessagesMessage, $MessagesError);
            }         // report swearing
            if (key_exists('delmes' . $msg_id, $_POST) && $_POST['delmes' . $msg_id] === 'on' && $_POST['deletemessages'] === 'deletemarked') {
                DeleteMessage($player_id, $msg_id);
            }    // Delete selected
            if (!key_exists('delmes' . $msg_id, $_POST) && $_POST['deletemessages'] === 'deletenonmarked') {
                DeleteMessage($player_id, $msg_id);
            }    // Delete unselected
...
while ($num--) {
    ...
    if ($pm == MTYP_PM) {
        echo '<tr><th colspan="4"><input type="checkbox" name="sneak' . $msg['msg_id'] . '"/><input type="submit" value="' . loca('MSG_REPORT') . "\"/></th></tr>\n";
    }
...
while ($rows--) {
    ...
    if ($oper['flags'] & USER_FLAG_HIDE_GO_EMAIL) {
        $href = 'index.php?page=writemessages&session=' . $_GET['session'] . '&messageziel=' . $oper['player_id'] . '&betreff=' . $subj;
    } else {
        $href = 'mailto:' . $oper['email'] . '?subject=' . $subj;
    }
