...
if ($GlobalUser['validated'] == 0) {
    ...
    if (method() === 'POST') {
        ...
        } elseif ($_POST['db_email'] !== $GlobalUser['email'] && $_POST['db_email'] !== '') {        // Change email address
...
<?php
      ...
      <?php
                if ($GlobalUser['disable']) {
                    echo 'am: ' . date('Y-m-d H:i:s', $GlobalUser['disable_until']) . "<input type='hidden' name=loeschen_am value=" . date('Y-m-d H:i:s', $GlobalUser['disable_until']) . '>';
                }
            ?>           </th>
              </tr>
<?php
...
<?php
    ...
    if (method() === 'POST' && !key_exists('urlaub_aus', $_POST)) {
        ...
        if ($GlobalUser['name_changed'] == 0 && $_POST['db_character'] !== $GlobalUser['oname']) {        // Change the name.
            ...
            } elseif (!CanChangeName($GlobalUser['player_id'])) {
                $OptionsError = loca('OPTIONS_ERR_NAME_WEEK');
            } elseif (mb_strlen($_POST['db_character']) < 3 || mb_strlen($_POST['db_character']) > 20) {
                $OptionsError = loca('OPTIONS_ERR_NAME_3_20');
            } elseif (preg_match('/[<>()\[\]{}\\\\\/\`\"\'.,:;*+]/', $_POST['db_character'])) {
        ...
        } elseif ($_POST['db_email'] !== $GlobalUser['pemail'] && $_POST['db_email'] !== '') {        // Change email address
            $email = $_POST['db_email'];
            if ($GlobalUser['password'] !== md5($_POST['db_password'] . $db_secret)) {
                $OptionsError = loca('OPTIONS_ERR_NEEDPASS');
            } elseif (!isValidEmail($email)) {
        ...
        if ($prem['commander']) {
            ...
            if ($settings_esp) {
                $flags |= USER_FLAG_SHOW_ESPIONAGE_BUTTON;
            } else {
                $flags &= ~USER_FLAG_SHOW_ESPIONAGE_BUTTON;
            }
            if ($settings_wri) {
                $flags |= USER_FLAG_SHOW_WRITE_MESSAGE_BUTTON;
            } else {
                $flags &= ~USER_FLAG_SHOW_WRITE_MESSAGE_BUTTON;
            }
            if ($settings_bud) {
                $flags |= USER_FLAG_SHOW_BUDDY_BUTTON;
            } else {
                $flags &= ~USER_FLAG_SHOW_BUDDY_BUTTON;
            }
            if ($settings_mis) {
                $flags |= USER_FLAG_SHOW_ROCKET_ATTACK_BUTTON;
            } else {
                $flags &= ~USER_FLAG_SHOW_ROCKET_ATTACK_BUTTON;
            }
            if ($settings_rep) {
                $flags |= USER_FLAG_SHOW_VIEW_REPORT_BUTTON;
            } else {
                $flags &= ~USER_FLAG_SHOW_VIEW_REPORT_BUTTON;
            }
            if ($settings_folders) {
                $flags |= USER_FLAG_DONT_USE_FOLDERS;
            } else {
                $flags &= ~USER_FLAG_DONT_USE_FOLDERS;
            }
            if ($flags & USER_FLAG_FEED_ENABLE && key_exists('feed_type', $_POST)) {
                if ($_POST['feed_type'] === 'atom') {
                    $flags |= USER_FLAG_FEED_ATOM;
                } else {
                    $flags &= ~USER_FLAG_FEED_ATOM;
                }
            }
