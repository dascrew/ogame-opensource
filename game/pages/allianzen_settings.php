...
function PageAlly_Settings()
{
    ...
    if (!key_exists('t', $_GET) || $_GET['t'] < 1 || $_GET['t'] > 3) {
        $_GET['t'] = 1;
    }
    ...
    if ($owner_name === loca('ALLY_NEW_RANK_FOUNDER')) {
        $owner_name = '';
    }
    ?>
...
<?php
        if ($_GET['t'] == 2) {
            echo loca('ALLY_SETTINGS_INTTEXT_DEF');
        } elseif ($_GET['t'] == 3) {
            echo loca('ALLY_SETTINGS_APPTEXT_DEF');
        } else {
            echo loca('ALLY_SETTINGS_EXTTEXT_DEF');
        }
    ?> (<span id="cntChars">
<?php
        if ($_GET['t'] == 2) {
