...
if (key_exists('qm', $_POST)) {
    $source_id = intval($_POST['qm']);
} else {
    $source_id = 0;
}
...
if ($source['type'] != PTYP_MOON) {
    $GateError .= "<center>\n" . loca('GATE_ERR_START') . "<br></center>\n";
}
if ($target['type'] != PTYP_MOON) {
    $GateError .= "<center>\n" . loca('GATE_ERR_TARGET') . "<br></center>\n";
}

if ($GateError === '') {
    if ($source['b' . GID_B_JUMP_GATE] == 0) {
        $GateError .= "<center>\n" . loca('GATE_ERR_START_GATE') . "<br></center>\n";
    }
    if ($target['b' . GID_B_JUMP_GATE] == 0) {
