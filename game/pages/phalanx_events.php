...
function OverFleet($fleet, $summary, $mission)
{
    ...
    foreach ($fleetmap as $i => $gid) {
        $amount = $fleet[$gid];
        if ($amount > 0) {
            $res .= loca("NAME_$gid") . ' ' . nicenum($amount) . '&lt;br&gt;';
        }
    }
...
function FleetSpan($fleet_entry)
{
    ...
    if ($mission == FTYP_ATTACK) {    // Attack
        if ($dir == 0) {
            echo "<span class='flight phalanx_fleet'>" . va(loca('EVENT_FLEET_ENEMY'), OverFleet($fleet, 1, 'phalanx_fleet')) . "</a><a href='#' title='" . TitleFleet($fleet, 1) . "'></a>" .
            ...
            echo "<span class='return phalanx_fleet'>" . va(loca('EVENT_FLEET_ENEMY'), OverFleet($fleet, 1, 'phalanx_fleet')) . "</a><a href='#' title='" . TitleFleet($fleet, 1) . "'></a>" .
        ...
        if ($dir == 0) {
            echo "<span class='phalanx_fleet'>" . va(loca('EVENT_FLEET_ENEMY'), OverFleet($fleet, 1, 'phalanx_fleet')) . "</a><a href='#' title='" . TitleFleet($fleet, 1) . "'></a>" .
        ...
        if ($dir == 2) {
            echo "<span class='holding phalanx_fleet'>" . va(loca('EVENT_FLEET_HOLD'), PlayerDetails($owner), OverFleet($fleet, 1, 'phalanx_fleet')) . "</a><a href='#' title='" . TitleFleet($fleet, 1) . "'></a>" .
    ...
    } elseif ($mission == FTYP_DESTROY) {    // Destroy (hmmm... how to see this on the phalanx is not clear, but we'll leave the code for unification.)
        if ($dir == 0) {
            echo "<span class='flight phalanx_fleet'>" . va(loca('EVENT_FLEET_ENEMY'), OverFleet($fleet, 1, 'phalanx_fleet')) . "</a><a href='#' title='" . TitleFleet($fleet, 1) . "'></a>" .
                va(loca('EVENT_FROM_TO_PHALANX'), PlayerDetails($owner), PlanetFrom($origin, 'phalanx_fleet'), PlanetTo($target, 'phalanx_fleet')) .
        ...
        if ($dir == 2) {
            echo "<span class='holding phalanx_fleet'>" . va(loca('EVENT_FLEET_FRIEND'), OverFleet($fleet, 1, 'phalanx_fleet')) . "</a><a href='#' title='" . TitleFleet($fleet, 1) . "'></a>" .
...
function PhalanxEventList($planet_id)
{
    ...
    while ($rows--) {
        ...
        if ($fleet_obj['mission'] == (FTYP_RETURN + FTYP_DEPLOY)) {
            continue;
        }
        if ($fleet_obj['mission'] == FTYP_DEPLOY && $fleet_obj['start_planet'] == $planet_id) {
            continue;
        }

        // Do not show fleets returning from the target planet.
        if (($fleet_obj['mission'] > FTYP_RETURN && $fleet_obj['mission'] < FTYP_ORBITING) && $fleet_obj['target_planet'] == $planet_id) {
            continue;
        }
        ...
        if ($fleet_obj['mission'] < FTYP_RETURN && $fleet_obj['start_planet'] == $planet_id) {
            if ($fleet_obj['mission'] != FTYP_EXPEDITION) {
        ...
        } elseif (GetMission($fleet_obj) == FTYP_ACS_HOLD) {
            if ($fleet_obj['mission'] < FTYP_RETURN) {
                $task[$tasknum]['fleet'][0]['dir'] = 0;
            } elseif ($fleet_obj['mission'] < FTYP_ORBITING) {
                $task[$tasknum]['fleet'][0]['dir'] = 1;
