...
function loca($key)
{
    ...
    if (!isset($LOCA[$loca_lang][$key])) {
        return $key;
    } else {
        return loca_subst_ogame($LOCA[$loca_lang][$key]);
    }
}
...
{
    global $LOCA;
    if (!isset($LOCA[$lang][$key])) {
        return $key;
    } else {
        return loca_subst_ogame($LOCA[$lang][$key]);
    }
}
...
{
    global $LOCA, $Languages, $DefaultLanguage;
    ...
    $path = 'loca/' . $lang . '_' . $lang . '/' . $section . '.php';
    if (file_exists($path)) {
        include_once $path;
        return;
    }

    // Fallback to default language if requested locale file is missing
    $fallback = 'loca/' . $DefaultLanguage . '_' . $DefaultLanguage . '/' . $section . '.php';
