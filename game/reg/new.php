...
if (method() === 'POST') {        // Register a player.
    ...
    if (!key_exists('agb', $_POST)) {
        ...
        $agbclass = 'error';
    } elseif (($now - $last) < 10 * 60 && !$localhost) {
        $error = loca('REG_NEW_ERROR_IP');
    } elseif (mb_strlen($_POST['character']) < 3 || mb_strlen($_POST['character']) > 20 || preg_match('/[;,<>()\`\"\']/', $_POST['character'])) {
        $error = va(loca('REG_NEW_ERROR_CHARS'), $_POST['character']);
    } elseif (IsUserExist($_POST['character'])) {
        $error = va(loca('REG_NEW_ERROR_EXISTS'), $_POST['character']) ;
    } elseif (!isValidEmail($_POST['email'])) {
        $error = va(loca('REG_NEW_ERROR_EMAIL'), $_POST['email']) ;
    } elseif (IsEmailExist($_POST['email'])) {
        $error = va(loca('REG_NEW_ERROR_EMAIL_EXISTS'), $_POST['email']);
    } elseif (GetUsersCount() >= $uni['maxusers']) {
        $error = va(loca('REG_NEW_ERROR_MAX_PLAYERS'), $uni['maxusers']);
    }
