...
function Admin_Planets()
{
    ...
    if (method() === 'POST' && $GlobalUser['admin'] >= 2) {
        ...
        if ($action === 'update') {        // Update the planet's data.
            ...
            if ($rows > 0) {
                ...
                $SearchResult .= loca('ADM_PLANET_NOT_FOUND') . "<br>\n";
            }
            $SearchResult .= "</table>\n";
        }
    }
    ...
    if (method() === 'GET' && $GlobalUser['admin'] >= 2) {
        ...
        } elseif ($action === 'create_debris') {    // Create debris field
            $planet = GetPlanet($cp);
            if ($planet['type'] > PTYP_MOON && $planet['type'] < PTYP_DF) {
    ...
    if (key_exists('cp', $_GET)) {     // Planet Information.
        ...
        // Update resources before displaying the planet info
        if (function_exists('ProdResources')) {
            $now = time();
            ProdResources($planet, $planet['lastpeek'], $now);
            // Reload planet data after update
            $planet = GetPlanet($planet['planet_id']);
        }
...
<?php
        ...
        if ($planet['type'] > PTYP_MOON && $planet['type'] < PTYP_DF) {
            if ($moon_id) {
                ...
                echo "<a href=\"index.php?page=admin&session=$session&mode=Planets&action=create_moon&cp=" . $planet['planet_id'] . '" >' . loca('ADM_PLANET_ADD_MOON') . "</a>\n";
            }
            echo "<br/><br/>\n";
            if ($debris_id) {
                ...
                echo "<a href=\"index.php?page=admin&session=$session&mode=Planets&action=create_debris&cp=" . $planet['planet_id'] . '" >' . loca('ADM_PLANET_ADD_DF') . "</a>\n";
            }
        } else {
