...
function Admin_BattleSim()
{
    ...
    if (method() === 'POST' && $GlobalUser['admin'] != 0) {
        ...
        // Simulate the battle
        $battle_result = 0;
        if (key_exists('debug', $_POST) && $_POST['debug'] === 'on') {
            $debug = true;
        } else {
            $debug = false;
        }
...
<tr><td><?=loca('ADM_SIM_DEBUG');?></td><td><input type="checkbox" name="debug" <?php if ($debug) {
    echo 'checked';
} ?> ></td></tr>
<tr><td><?=loca('ADM_SIM_RAPIDFIRE');?></td><td><input type="checkbox" name="rapid" <?php if ($rf) {
    echo 'checked';
} ?> ></td></tr>
