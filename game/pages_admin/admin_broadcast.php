...
function Admin_Broadcast()
{
    ...
    if (method() === 'POST') {
        ...
        $subj = $_POST['subj'];
        if ($subj === '') {
            $write_error = '<center><font color=#FF0000>' . loca('ADM_BCAST_ERROR_SUBJ') . "</font><br/></center>\n";
        }
        $text = $_POST['text'];
        if ($text === '') {
            $write_error = '<center><font color=#FF0000>' . loca('ADM_BCAST_ERROR_TEXT') . "</font><br/></center>\n";
        }

        if ($write_error === '') {
            if ($cat == 1) {
                $query = 'SELECT * FROM ' . $db_prefix . 'users WHERE score1 < ' . USER_NOOB_LIMIT . ';';
            }        // Newbies (usually less than 5.000 points)
            elseif ($cat == 2) {
            ...
            if ($usernum > 0) {
                $write_error = '<center><font color=#00FF00>' . va(loca('ADM_BCAST_SUCCESS'), $usernum) . "</font><br/></center>\n";
            } else {
                $write_error = '<center><font color=#00FF00>' . loca('ADM_BCAST_ERROR_USERS') . "</font><br/></center>\n";
            }
        }
    }
