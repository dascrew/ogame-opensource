...
function QueueDesc($queue)
{
    ...
    switch ($type) {
        ...
        case QTYP_AI:
            ...
            // Handle IDLE state display
            if ($block_id === 'IDLE') {
                $next_block_id = $queue['level'];
                $idle_duration = $queue['end'] - time();
                if ($strat_id > 0) {
                    $query = 'SELECT name FROM ' . $db_prefix . "botstrat WHERE id = $strat_id LIMIT 1;";
                    $result = dbquery($query);
                    $strat = dbarray($result);
                    $strat_name = isset($strat['name']) ? $strat['name'] : 'Unknown';
                    return va(loca('ADM_QUEUE_TYPE_AI'), $strat_name) . ' : <br><strong>IDLE</strong> (' . max(0, $idle_duration) . 's remaining)';
                }
                return loca('ADM_QUEUE_TYPE_AI') . ' : <br><strong>IDLE</strong>';
            }
            ...
            $strat = dbarray($result);
            $block_text = '';
            if ($strat && !empty($strat['source'])) {
                $source = json_decode($strat['source'], true);
                if (is_array($source) && isset($source['nodeDataArray']) && is_array($source['nodeDataArray'])) {
                    foreach ($source['nodeDataArray'] as $i => $arr) {
                        if (isset($arr['key']) && $arr['key'] == $block_id && isset($arr['text'])) {
                            $block_text = $arr['text'];
                            break;
                        }
                    }
            ...
            return va(loca('ADM_QUEUE_TYPE_AI'), isset($strat['name']) ? $strat['name'] : '') . ' : <br>' . htmlspecialchars($block_text);
...
function Admin_Queue()
{
    ...
    if ($player_id > 0) {
        $query = 'SELECT * FROM ' . $db_prefix . "queue WHERE (type <> '" . QTYP_FLEET . "' AND type <> '" . QTYP_COMMANDER_OFF . "') AND owner_id=$player_id ORDER BY end ASC, prio DESC";
    } else {
        $query = 'SELECT * FROM ' . $db_prefix . "queue WHERE (type <> '" . QTYP_FLEET . "' AND type <> '" . QTYP_COMMANDER_OFF . "') ORDER BY end ASC, prio DESC LIMIT 50";
    }
    $result = dbquery($query);
    $now = time();
