...
function Admin_Coupons()
{
    ...
    if (method() === 'POST' && $GlobalUser['admin'] >= 2) {
        ...
        if ($action === 'add_one') {
            ...
            if ($code == null) {
                $AdminError = '<font color=red>' . loca('ADM_COUPON_ERROR') . '</font>';
            } else {
                $AdminMessage = '<font color=lime>' . va(loca('ADM_COUPON_SUCCESS'), $code) . '</font>';
            }
        }
        ...
        if ($action === 'add_date') {
            ...
            $queue = [ null, USER_SPACE, QTYP_COUPON, $darkmatter, ($inactive_days << 16) | $ingame_days, $periodic, $now, $end, QUEUE_PRIO_COUPON ];
...
<?php
    ...
    if ($from >= $count) {
        echo '     <a href="' . $url . '=' . ($from - $count) . '"><< ' . va(loca('ADM_COUPON_PREV'), $count) . "</a>&nbsp;&nbsp;&nbsp;&nbsp;\n";
    }
    if ($from < $total && ($from + $count < $total)) {
        echo '        <a href="' . $url . '=' . ($from + $count) . '">' . va(loca('ADM_COUPON_NEXT'), $count) . " >></a>\n";
    }
    ?>
      </th>
   </tr>
