    ...
    if (method() === 'POST' && $GlobalUser['admin'] >= 2) {
        if (BotStrategyExists('_start')) {
            if (AddBot($_POST['name'])) {
                $msg = urlencode(loca('ADM_BOTS_ADDED'));
                header("Location: index.php?page=admin&session=$session&mode=Bots&msg=$msg");
                exit();
            } else {
                $result = '<font color=red>' . loca('ADM_BOTS_USER_NOT_FOUND') . '</font>';
            }
        } else {
...
<?php
if (isset($_GET['msg'])) {
    echo '<div style="text-align:center;color:lime">' . htmlspecialchars($_GET['msg']) . '</div>';
}
    ?>
<?=$result;?>
...
<?php
    ...
    if ($rows == 0) {
        echo loca('ADM_BOTS_NOT_FOUND') . '<br>';
    } else {
        echo "<table>\n";
