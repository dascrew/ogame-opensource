...
$FleetMissionList = [
    0 => [ 1, 101, 2, 102, 3, 103, 4, 104, 5, 105, 205, 6, 106, 7, 107, 8, 108, 9, 109, 15, 115, 215, 20, 21, 121 ],
    1 => [ 1, 101 ],
    2 => [ 2, 102 ],
    3 => [ 3, 103 ],
    4 => [ 4, 104 ],
...
function LinkFleetsFrom($user, $mission)
{
    ...
    if ($rows) {
        return '<a href="index.php?page=admin&session=' . $session . '&mode=Users&action=fleetlogs&player_id=' . $user['player_id'] . '&mission=' . $mission . '&from=1">' . $rows . '</a>';
    } else {
        return '0';
    }
}
...
function LinkFleetsTo($user, $mission)
{
    ...
    if ($rows) {
        return '<a href="index.php?page=admin&session=' . $session . '&mode=Users&action=fleetlogs&player_id=' . $user['player_id'] . '&mission=' . $mission . '&from=0">' . $rows . '</a>';
    } else {
        return '0';
    }
}
...
function Admin_Users()
{
    ...
    if (method() === 'POST' && $GlobalUser['admin'] >= 2) {
        ...
        if ($action === 'update') {        // Update user data.
            ...
            if (key_exists('banned', $_POST) && $_POST['banned'] !== 'on') {
                $query .= 'banned = 0, ';
            }
            if (key_exists('noattack', $_POST) && $_POST['noattack'] !== 'on') {
                $query .= 'noattack = 0, ';
            }
            ...
            $qname = [ QTYP_COMMANDER_OFF, QTYP_ADMIRAL_OFF, QTYP_ENGINEER_OFF, QTYP_GEOLOGE_OFF, QTYP_TECHNOCRATE_OFF ];
...
<?php
            $oname = [ loca('PR_COMA'), loca('PR_ADMIRAL'), loca('PR_ENGINEER'), loca('PR_GEOLOGIST'), loca('PR_TECHNO') ];
        $odesc = [ '',
        ...
        $qname = [ QTYP_COMMANDER_OFF, QTYP_ADMIRAL_OFF, QTYP_ENGINEER_OFF, QTYP_GEOLOGE_OFF, QTYP_TECHNOCRATE_OFF ];
        $imgname = [ 'commander', 'admiral', 'ingenieur', 'geologe', 'technokrat'];
