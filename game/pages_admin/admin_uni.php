...
<?php
        if ($now > $unitab['news_until']) {
            echo '<tr><th>' . loca('ADM_UNI_NEWS_PROLONG') . '</th><th><input type="text" name="news_upd" maxlength="3" size="3" value="0" /> ' . loca('ADM_UNI_DAYS') . "</th></tr>\n";
        } else {
            echo '<tr><th>' . loca('ADM_UNI_NEWS_SHOW_UNTIL') . '</th><th>' . date('Y-m-d H:i:s', $unitab['news_until']) . ' <input type="checkbox" name="news_off"  /> ' . loca('ADM_UNI_NEWS_REMOVE') . "</th></tr>\n";
        }
    ?>
