...
function Error($text)
{
    ...
    $error = [ null, $GlobalUser['player_id'], $_SERVER['REMOTE_ADDR'], $_SERVER['HTTP_USER_AGENT'], $_SERVER['REQUEST_URI'], $text, $now ];
...
function Debug($message)
{
    ...
    $error = [ null, $GlobalUser['player_id'], $_SERVER['REMOTE_ADDR'], $_SERVER['HTTP_USER_AGENT'], $_SERVER['REQUEST_URI'], $message, $now ];
...
function BrowseHistory()
{
    ...
    if ($GlobalUser['sniff']) {
        ...
        $log = [ null, $GlobalUser['player_id'], $_SERVER['REQUEST_URI'], $_SERVER['REQUEST_METHOD'], $getdata, $postdata, time() ];
        AddDBRow($log, 'browse');
    }
}
...
function SecurityCheck($match, $text, $notes)
{
    ...
    if (!preg_match($match, $text)) {
        Error(loca_lang('DEBUG_SECURITY_BREACH', $GlobalUni['lang']) . $notes);
    }
}
...
function UserLog($owner_id, $type, $text, $when = 0)
{
    ...
    $log = [ null, $owner_id, $when, $type, $text ];
