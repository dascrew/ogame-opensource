...
function ExpUpperLimit($exptab)
{
    ...
    if ($user) {
        ...
        if ($score < $exptab['score_cap1']) {
            return $exptab['limit_cap1'];
        }
        if ($score < $exptab['score_cap2']) {
            return $exptab['limit_cap2'];
        }
        if ($score < $exptab['score_cap3']) {
            return $exptab['limit_cap3'];
        }
        if ($score < $exptab['score_cap4']) {
            return $exptab['limit_cap4'];
        }
        if ($score < $exptab['score_cap5']) {
            return $exptab['limit_cap5'];
        }
        if ($score < $exptab['score_cap6']) {
            return $exptab['limit_cap6'];
        }
        if ($score < $exptab['score_cap7']) {
            return $exptab['limit_cap7'];
        }
        if ($score < $exptab['score_cap8']) {
            return $exptab['limit_cap8'];
        }
...
function Logbook($expcount, $exptab, $lang)
{
    ...
    if ($expcount <= $exptab['depleted_min']) {
        ...
        return $msg_1[$n];
    } elseif ($expcount <= $exptab['depleted_med']) {
        $n = mt_rand(0, count($msg_2) - 1);
        return $msg_2[$n];
    } elseif ($expcount <= $exptab['depleted_max']) {
...
function Exp_ResourcesFound($exptab, $queue, $fleet_obj, $fleet, $origin, $target, $lang)
{
    ...
    $resname = [ loca_lang('METAL', $lang), loca_lang('CRYSTAL', $lang), loca_lang('DEUTERIUM', $lang) ];
...
function Exp_FleetFound($exptab, $queue, $fleet_obj, $fleet, $origin, $target, $lang)
{
    ...
    if ($fleet[GID_F_PROBE] > 0) {
        $found = [ GID_F_PROBE, GID_F_SC ];
    }    // Espionage Probe
    if ($fleet[GID_F_SC] > 0) {
        $found = [ GID_F_PROBE, GID_F_SC, GID_F_LC ];
    }    // Small Cargo
    if ($fleet[GID_F_LF] > 0) {
        $found = [ GID_F_PROBE, GID_F_SC, GID_F_LF, GID_F_LC ];
    }    // Light Fighter
    if ($fleet[GID_F_LC] > 0) {
        $found = [ GID_F_PROBE, GID_F_SC, GID_F_LF, GID_F_LC, GID_F_HF ];
    }    // Large Cargo
    if ($fleet[GID_F_HF] > 0) {
        $found = [ GID_F_PROBE, GID_F_SC, GID_F_LF, GID_F_LC, GID_F_HF, GID_F_CRUISER ];
    }    // Heavy Fighter
    if ($fleet[GID_F_CRUISER] > 0) {
        $found = [ GID_F_PROBE, GID_F_SC, GID_F_LF, GID_F_LC, GID_F_HF, GID_F_CRUISER, GID_F_BATTLESHIP ];
    }    // Cruiser
    if ($fleet[GID_F_BATTLESHIP] > 0) {
        $found = [ GID_F_PROBE, GID_F_SC, GID_F_LF, GID_F_LC, GID_F_HF, GID_F_CRUISER, GID_F_BATTLESHIP, GID_F_BATTLECRUISER ];
    }     // Battleship
    if ($fleet[GID_F_BATTLECRUISER] > 0) {
        $found = [ GID_F_PROBE, GID_F_SC, GID_F_LF, GID_F_LC, GID_F_HF, GID_F_CRUISER, GID_F_BATTLESHIP, GID_F_BATTLECRUISER, GID_F_BOMBER ];
    }    // Battlecruiser
    if ($fleet[GID_F_BOMBER] > 0) {
        $found = [ GID_F_PROBE, GID_F_SC, GID_F_LF, GID_F_LC, GID_F_HF, GID_F_CRUISER, GID_F_BATTLESHIP, GID_F_BATTLECRUISER, GID_F_BOMBER, GID_F_DESTRO ];
    }    // Bomber
    if ($fleet[GID_F_DESTRO] > 0) {
        $found = [ GID_F_PROBE, GID_F_SC, GID_F_LF, GID_F_LC, GID_F_HF, GID_F_CRUISER, GID_F_BATTLESHIP, GID_F_BATTLECRUISER, GID_F_BOMBER, GID_F_DESTRO ];
    }    // Destroyer
    ...
    foreach ($found_ids as $i => $id) {
        ...
        if ($amount == 0) {
            $no_structure = true;
            break;
        }    // there wasn't enough structure for the rest of the fleet.
    ...
    if ($chance < ($exptab['chance_success'] + $hold_time)) {
        ...
        if ($chance >= $chance_depleted) {    // Successful expedition.
            if ($chance >= $exptab['chance_alien']) {
                $res = EXP_ALIENS;
            } elseif ($chance >= $exptab['chance_pirates']) {
                $res = EXP_PIRATES;
            } elseif ($chance >= $exptab['chance_dm']) {
                $res = EXP_DARK_MATTER;
            } elseif ($chance >= $exptab['chance_lost']) {
                $res = EXP_BLACK_HOLE;
            } elseif ($chance >= $exptab['chance_delay']) {
                $res = EXP_DELAY;
            } elseif ($chance >= $exptab['chance_accel']) {
                $res = EXP_ACCEL;
            } elseif ($chance >= $exptab['chance_res']) {
                $res = EXP_RESOURCES;
            } elseif ($chance >= $exptab['chance_fleet']) {
                $res = EXP_FLEET;
            } else {
                $res = EXP_TRADER;
            }
