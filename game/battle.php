...
{
    $repaired = [ 401 => 0, 402 => 0, 403 => 0, 404 => 0, 405 => 0, 406 => 0, 407 => 0, 408 => 0 ];
    $exploded = [ 401 => 0, 402 => 0, 403 => 0, 404 => 0, 405 => 0, 406 => 0, 407 => 0, 408 => 0 ];
    $exploded_total = 0;
    ...
    if ($rounds > 0) {
        ...
        foreach ($exploded as $gid => $amount) {
            ...
            if (key_exists('engineer', $prem) && $prem['engineer']) {
...
function Plunder($cargo, $m, $k, $d)
{
    ...
    $res = [ 'cm' => floor($mc), 'ck' => floor($kc), 'cd' => floor($dc) ];
    return $res;
}
...
function CalcLosses($a, $d, $res, $repaired)
{
    ...
    return  [ 'a' => $a, 'd' => $d, 'aloss' => $aloss, 'dloss' => $dloss ];
}
...
function WritebackBattleResults($a, $d, $res, $repaired, $cm, $ck, $cd, $sum_cargo)
{
    ...
    if ($rounds > 0) {
        ...
        foreach ($last['attackers'] as $i => $attacker) {        // Attackers
            ...
            if ($ships > 0) {
                if ($fleet_obj['mission'] == FTYP_DESTROY && $res['result'] === 'awon') {
                    $result = GravitonAttack($fleet_obj, $attacker, $queue['end']);
                } else {
                    $result = 0;
                }
                if ($result < 2) {
                    DispatchFleet($attacker, $origin, $target, $fleet_obj['mission'] + FTYP_RETURN, $fleet_obj['flight_time'], $fleet_obj['m'] + $cm * $cargo, $fleet_obj['k'] + $ck * $cargo, $fleet_obj['d'] + $cd * $cargo, $fleet_obj['fuel'] / 2, $queue['end']);
                }
            }
        }
    ...
    // Combat with no rounds.
        ...
        foreach ($a as $i => $attacker) {            // Attackers
            ...
            if ($ships > 0) {
                ...
                if ($result < 2) {
                    DispatchFleet($attacker['fleet'], $origin, $target, $fleet_obj['mission'] + FTYP_RETURN, $fleet_obj['flight_time'], $fleet_obj['m'] + $cm * $cargo, $fleet_obj['k'] + $ck * $cargo, $fleet_obj['d'] + $cd * $cargo, $fleet_obj['fuel'] / 2, $queue['end']);
                }
            }
        }
...
function GenSlot($weap, $shld, $armor, $name, $g, $s, $p, $unitmap, $fleet, $defense, $show_techs, $attack, $lang)
{
    ...
    $text .= '<center>';
    if ($attack) {
        $text .= loca_lang('BATTLE_ATTACKER', $lang);
    } else {
        $text .= loca_lang('BATTLE_DEFENDER', $lang);
    }
    $text .= ' ' . $name . " (<a href=# onclick=showGalaxy($g,$s,$p); >[$g:$s:$p]</a>)";
    if ($show_techs) {
        $text .= '<br>' . loca_lang('BATTLE_ATTACK', $lang) . ' ' . ($weap * 10) . '% ' . loca_lang('BATTLE_SHIELD', $lang) . ' ' . ($shld * 10) . '% ' . loca_lang('BATTLE_ARMOR', $lang) . ' ' . ($armor * 10) . '% ';
    }
    ...
    if ($sum > 0) {
        ...
        foreach ($unitmap as $i => $gid) {
            ...
            if ($n > 0) {
                $text .= '<th>' . loca_lang("SNAME_$gid", $lang) . '</th>';
            }
        }
        $text .= '</tr>';
        ...
        foreach ($unitmap as $i => $gid) {
            ...
            if ($n > 0) {
                $text .= '<th>' . nicenum($UnitParam[$gid][2] * (10 + $weap) / 10) . '</th>';
            }
        }
        $text .= '</tr>';
        ...
        foreach ($unitmap as $i => $gid) {
            ...
            if ($n > 0) {
                $text .= '<th>' . nicenum($UnitParam[$gid][1] * (10 + $shld) / 10) . '</th>';
            }
        }
        $text .= '</tr>';
        ...
        foreach ($unitmap as $i => $gid) {
            ...
            if ($n > 0) {
                $text .= '<th>' . nicenum($UnitParam[$gid][0] * (10 + $armor) / 100) . '</th>';
            }
        }
        $text .= '</tr>';
        ...
        $text .= '<br>' . loca_lang('BATTLE_DESTROYED', $lang);
    }
...
function BattleReport($res, $now, $aloss, $dloss, $cm, $ck, $cd, $moonchance, $mooncreated, $repaired, $lang)
{
    ...
    foreach ($res['rounds'] as $i => $round) {
        ...
        foreach ($round['defenders'] as $n => $defender) {
            if ($n == 0) {
                ...
                $text .= GenSlot(0, 0, 0, $defender['name'], $defender['g'], $defender['s'], $defender['p'], $amap, $defender, null, 0, 0, $lang);
            }
        }
        $text .= '</tr></table>';
    }
    ...
    } elseif ($res['result'] === 'dwon') {
        $text .= '<p> ' . loca_lang('BATTLE_DWON', $lang);
    } elseif ($res['result'] === 'draw') {
        $text .= '<p> ' . loca_lang('BATTLE_DRAW', $lang);
    }
    ...
    if ($moonchance) {
        $text .= '<br>' . va(loca_lang('BATTLE_MOONCHANCE', $lang), $moonchance);
    }
    if ($mooncreated) {
        $text .= '<br>' . loca_lang('BATTLE_MOON', $lang);
    }
    ...
    $repairmap = [ GID_D_RL, GID_D_LL, GID_D_HL, GID_D_GAUSS, GID_D_ION, GID_D_SDOME, GID_D_PLASMA, GID_D_LDOME ];
    $repaired_num = $sum = 0;
    foreach ($repaired as $gid => $amount) {
        $repaired_num += $amount;
    }
    if ($repaired_num > 0) {
        $text .= '<br>';
...
function GravitonAttack($fleet_obj, $fleet, $when)
{
    ...
    if ($fleet[GID_F_DEATHSTAR] == 0) {
        return;
    }
    if (! ($target['type'] == PTYP_MOON || $target['type'] == PTYP_DEST_MOON)) {
    ...
    if (!$ripdes && !$moondes) {
        $atext = va(
            loca_lang('GRAVITON_ATK_00', $origin_user['lang']),
            $origin['name'],
            '[' . $origin['g'] . ':' . $origin['s'] . ':' . $origin['p'] . ']',
            '[' . $target['g'] . ':' . $target['s'] . ':' . $target['p'] . ']',
            floor($moonchance),
            floor($ripchance)
        );
        $dtext = va(
            loca_lang('GRAVITON_DEF_00', $target_user['lang']),
            $origin['name'],
            '[' . $origin['g'] . ':' . $origin['s'] . ':' . $origin['p'] . ']',
            '[' . $target['g'] . ':' . $target['s'] . ':' . $target['p'] . ']',
            $origin['name'],
        ...
        $result = 0;
    } elseif (!$ripdes && $moondes) {
        $atext = va(
            loca_lang('GRAVITON_ATK_01', $origin_user['lang']),
            $origin['name'],
            '[' . $origin['g'] . ':' . $origin['s'] . ':' . $origin['p'] . ']',
            '[' . $target['g'] . ':' . $target['s'] . ':' . $target['p'] . ']',
            floor($moonchance),
        ...
        $dtext = va(
            loca_lang('GRAVITON_DEF_01', $target_user['lang']),
            $origin['name'],
            '[' . $origin['g'] . ':' . $origin['s'] . ':' . $origin['p'] . ']',
            '[' . $target['g'] . ':' . $target['s'] . ':' . $target['p'] . ']',
            floor($moonchance),
        ...
        $atext = va(
            loca_lang('GRAVITON_ATK_10', $origin_user['lang']),
            $origin['name'],
            '[' . $origin['g'] . ':' . $origin['s'] . ':' . $origin['p'] . ']',
            '[' . $target['g'] . ':' . $target['s'] . ':' . $target['p'] . ']',
            floor($moonchance),
        ...
        $dtext = va(
            loca_lang('GRAVITON_DEF_10', $target_user['lang']),
            $origin['name'],
            '[' . $origin['g'] . ':' . $origin['s'] . ':' . $origin['p'] . ']',
            '[' . $target['g'] . ':' . $target['s'] . ':' . $target['p'] . ']',
            floor($moonchance),
        ...
        $atext = va(
            loca_lang('GRAVITON_ATK_11', $origin_user['lang']),
            $origin['name'],
            '[' . $origin['g'] . ':' . $origin['s'] . ':' . $origin['p'] . ']',
            '[' . $target['g'] . ':' . $target['s'] . ':' . $target['p'] . ']',
            floor($moonchance),
        ...
        $dtext = va(
            loca_lang('GRAVITON_DEF_11', $target_user['lang']),
            $origin['name'],
            '[' . $origin['g'] . ':' . $origin['s'] . ':' . $origin['p'] . ']',
            '[' . $target['g'] . ':' . $target['s'] . ':' . $target['p'] . ']',
            floor($moonchance),
...
function StartBattle($fleet_id, $planet_id, $when)
{
    ...
    $a_result = [ 0 => 'combatreport_ididattack_iwon', 1 => 'combatreport_ididattack_ilost', 2 => 'combatreport_ididattack_draw' ];
    $d_result = [ 1 => 'combatreport_igotattacked_iwon', 0 => 'combatreport_igotattacked_ilost', 2 => 'combatreport_igotattacked_draw' ];
    ...
    if (PlanetHasMoon($planet_id) || $p['type'] == PTYP_MOON || $p['type'] == PTYP_DEST_MOON) {
        $moonchance = 0;
    }
    ...
    foreach ($d as $i => $user) {        // Defenders
        ...
        if (key_exists($user['lang'], $battle_text)) {
            $text = $battle_text[$user['lang']];
        } else {
        ...
        if (key_exists($user['player_id'], $mailbox)) {
            continue;
        }
    ...
    foreach ($a as $i => $user) {        // Attackers
        ...
        if (count($res['rounds']) <= 2 && $battle_result == 1) {
            $text = loca_lang('BATTLE_LOST', $user['lang']) . " <!--A:$aloss,W:$dloss-->";
        }
...
function WritebackBattleResultsExpedition($a, $d, $res)
{
    ...
    if ($rounds > 0) {
        ...
        foreach ($last['attackers'] as $i => $attacker) {        // Attackers
            ...
            if ($ships > 0) {
                DispatchFleet($attacker, $origin, $target, FTYP_EXPEDITION + FTYP_RETURN, $fleet_obj['deploy_time'], $fleet_obj['m'], $fleet_obj['k'], $fleet_obj['d'], $fleet_obj['fuel'] / 2, $queue['end']);
            }
        }
    ...
    // Combat with no rounds.
        ...
        foreach ($a as $i => $attacker) {            // Attackers
            ...
            // The hold time is used as the flight time.
            if ($ships > 0) {
                DispatchFleet($attacker['fleet'], $origin, $target, FTYP_EXPEDITION + FTYP_RETURN, $fleet_obj['deploy_time'], $fleet_obj['m'], $fleet_obj['k'], $fleet_obj['d'], $fleet_obj['fuel'] / 2, $queue['end']);
            }
        }
...
function ShortBattleReport($res, $now, $lang)
{
    ...
    foreach ($res['before']['defenders'] as $i => $defender) {
        ...
        $user['fleet'] = [];
        $user['defense'] = [];
        foreach ($fleetmap as $g => $gid) {
            $user['fleet'][$gid] = $defender[$gid];
        }
        foreach ($defmap as $g => $gid) {
            $user['defense'][$gid] = 0;
        }
        $text .= GenSlot($defender['weap'], $defender['shld'], $defender['armr'], $defender['name'], $defender['g'], $defender['s'], $defender['p'], $dmap, $defender, $defender, 1, 0, $lang);
    }
...
function ExpeditionBattle($fleet_id, $pirates, $level, $when)
{
    ...
    if ($pirates) {
        if ($level == 0) {
            $d[0]['fleet'][GID_F_LF] += 5;
        } elseif ($level == 1) {
            $d[0]['fleet'][GID_F_CRUISER] += 3;
        } elseif ($level == 2) {
            $d[0]['fleet'][GID_F_BATTLESHIP] += 2;
        }
    } else {
        if ($level == 0) {
            $d[0]['fleet'][GID_F_HF] += 5;
        } elseif ($level == 1) {
            $d[0]['fleet'][GID_F_BATTLECRUISER] += 3;
        } elseif ($level == 2) {
            $d[0]['fleet'][GID_F_DESTRO] += 2;
        }
    }
    ...
    // Calculate total losses (account for deuterium and repaired defenses)
    $aloss = $dloss = 0;
    $repaired = [ GID_D_RL => 0, GID_D_LL => 0, GID_D_HL => 0, GID_D_GAUSS => 0, GID_D_ION => 0, GID_D_PLASMA => 0, GID_D_SDOME => 0, GID_D_LDOME => 0 ];
    $loss = CalcLosses($a, $d, $res, $repaired);
    $a = $loss['a'];
    $d = $loss['d'];
    $aloss = $loss['aloss'];
