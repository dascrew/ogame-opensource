...
function BotCanBuild($obj_id)
{
    ...
    if ($text !== '') {
        ...
        BotDebug(
            "BotCanBuild: cannot build GID {$obj_id} level {$level} on planet {$aktplanet['planet_id']} ({$text}) M:$resM K:$resK D:$resD",
            'INFO'
        );
    }
    return $text === '';
...
function BotBuild($obj_id)
{
    ...
    if ($text === '') {
        ...
        BotDebug("Adding building GID $obj_id level $level to queue, duration: {$duration}s", 'ACTION');
        BuildEnque($user, $user['aktplanet'], $obj_id, 0, $BotNow);
        UpdatePlanetActivity($user['aktplanet'], time());
        UpdateLastClick($BotID);
        return $duration;
    } else {
        BotDebug("Cannot build GID $obj_id level $level: $text", 'INFO');
        return 0;
    }
}
...
function BotResourceSettings($last1 = 100, $last2 = 100, $last3 = 100, $last4 = 100, $last12 = 100, $last212 = 100)
{
    ...
    UpdatePlanetActivity($planet_id, time());
    UpdateLastClick($BotID);
}
