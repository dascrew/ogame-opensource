...
{
    $queue = [ null, $owner_id, $type, $sub_id, $obj_id, $level, $now, $now + $seconds, $prio ];
...
function UpdateQueue($until)
{
    ...
    while ($rows--) {
        ...
        switch ($queue['type']) {
            case QTYP_BUILD: Queue_Build_End($queue);
                break;
            case QTYP_DEMOLISH: Queue_Build_End($queue);
                break;
            case QTYP_RESEARCH: Queue_Research_End($queue);
                break;
            case QTYP_SHIPYARD: Queue_Shipyard_End($queue);
                break;
            case QTYP_FLEET: Queue_Fleet_End($queue);
                break;
            case QTYP_UNLOAD_ALL: Queue_Relogin_End($queue);
                break;
            case QTYP_CLEAN_DEBRIS: Queue_CleanDebris_End($queue);
                break;
            case QTYP_CLEAN_PLANETS: Queue_CleanPlanets_End($queue);
                break;
            case QTYP_CLEAN_PLAYERS: Queue_CleanPlayers_End($queue);
                break;
            case QTYP_UPDATE_STATS: Queue_UpdateStats_End($queue);
                break;
            case QTYP_RECALC_POINTS: Queue_RecalcPoints_End($queue);
                break;
            case QTYP_RECALC_ALLY_POINTS: Queue_RecalcAllyPoints_End($queue);
                break;
            case QTYP_ALLOW_NAME: Queue_AllowName_End($queue);
                break;
            case QTYP_CHANGE_EMAIL: Queue_ChangeEmail_End($queue);
                break;
            case QTYP_UNBAN: Queue_UnbanPlayer_End($queue);
                break;
            case QTYP_ALLOW_ATTACKS: Queue_AllowAttacks_End($queue);
                break;
            case QTYP_DEBUG: Queue_Debug_End($queue);
                break;
            case QTYP_AI: Queue_Bot_End($queue);
                break;
            case QTYP_COUPON: break;

            case QTYP_COMMANDER_OFF: Queue_Officer_End($queue);
                break;
            case QTYP_ADMIRAL_OFF: Queue_Officer_End($queue);
                break;
            case QTYP_ENGINEER_OFF: Queue_Officer_End($queue);
                break;
            case QTYP_GEOLOGE_OFF: Queue_Officer_End($queue);
                break;
            case QTYP_TECHNOCRATE_OFF: Queue_Officer_End($queue);
                break;
...
function CanBuild($user, $planet, $id, $lvl, $destroy, $enqueue = false)
{
    ...
    if ($GlobalUni['freeze']) {
        return loca_lang('BUILD_ERROR_UNI_FREEZE', $user['lang']);
    }

    // Not a building
    if (! in_array($id, $buildmap)) {
        return loca_lang('BUILD_ERROR_INVALID_ID', $user['lang']);
    }

    // You can't build in vacation mode
    elseif ($user['vacation']) {
        return loca_lang('BUILD_ERROR_VACATION_MODE', $user['lang']);
    }

    // You can't build on a foreign planet
    elseif ($planet['owner_id'] != $user['player_id']) {
    ...
    elseif ($planet['type'] != PTYP_MOON && ($id == GID_B_LUNAR_BASE || $id == GID_B_PHALANX || $id == GID_B_JUMP_GATE)) {
        return loca_lang('BUILD_ERROR_INVALID_PTYPE', $user['lang']);
    } elseif ($planet['type'] == PTYP_MOON && (
        $id == GID_B_METAL_MINE ||
        $id == GID_B_CRYS_MINE ||
        $id == GID_B_DEUT_SYNTH ||
        $id == GID_B_SOLAR ||
    ...
    elseif ($planet['fields'] >= $planet['maxfields'] && !$destroy) {
        return loca_lang('BUILD_ERROR_NO_SPACE', $user['lang']);
    }

    // Research or construction at the shipyard is underway
    elseif ($id == GID_B_RES_LAB && $reslab_operating) {
        return loca_lang('BUILD_ERROR_RESEARCH_ACTIVE', $user['lang']);
    } elseif (($id == GID_B_NANITES || $id == GID_B_SHIPYARD) && $shipyard_operating) {
        return loca_lang('BUILD_ERROR_SHIPYARD_ACTIVE', $user['lang']);
    }

    // Check the available amount of resources on the planet
    elseif (!IsEnoughResources($planet, $m, $k, $d, $e) && !$enqueue) {
        return loca_lang('BUILD_ERROR_NO_RES', $user['lang']);
    }

    // Check available technologies.
    elseif (!checkRequirements($id, $user, $planet)['met']) {
        return loca_lang('BUILD_ERROR_REQUIREMENTS', $user['lang']);
    }

    if ($destroy) {
        if ($id == GID_B_TERRAFORMER || $id == GID_B_LUNAR_BASE) {
            return loca_lang('BUILD_ERROR_CANT_DEMOLISH', $user['lang']);
        } elseif ($planet['b' . $id] <= 0) {
            return loca_lang('BUILD_ERROR_NO_SUCH_BUILDING', $user['lang']);
        }
    }
    ...
    if ($cnt > 0) {
        while ($row = dbarray($result)) {
            ...
            if ($text === '') {
                ...
                if ($destroy) {
                    $BuildEvent = QTYP_DEMOLISH;
                } else {
                    $BuildEvent = QTYP_BUILD;
                }
                ...
                if ($destroy) {
                    $pre = loca_lang('BUILD_MSG_DEMOLISH', $user['lang']);
                } else {
                    $pre = loca_lang('BUILD_MSG_BUILD', $user['lang']);
                }
...
function BuildEnque($user, $planet_id, $id, $destroy, $now = 0)
{
    ...
    if ($destroy) {
        UserLog($planet['owner_id'], 'BUILD', va(loca_lang('DEBUG_LOG_DEMOLISH', $GlobalUni['lang']), loca("NAME_$id"), $planet['b' . $id] - 1, $planet_id));
    } else {
        UserLog($planet['owner_id'], 'BUILD', va(loca_lang('DEBUG_LOG_BUILD', $GlobalUni['lang']), loca("NAME_$id"), $planet['b' . $id] + 1, $planet_id));
    }
    ...
    if ($cnt >= $maxcnt) {
        return '';
    }    // The queue of buildings is full
    ...
    for ($i = 0; $i < $cnt; $i++) {
        ...
        $usedStartTimes[$queue[$i]['start']] = true;
    }

    // Ensure the new queue entry has a unique start timestamp to avoid silent drops
    while (isset($usedStartTimes[$now])) {
        $now++;
    }
    ...
    if ($lvl < 0) {
        return '';
    }     // Unable to build/demolish a negative level
    ...
    if ($text === '') {
        ...
        $duration = floor(BuildDuration($id, $lvl, $planet['b' . GID_B_ROBOTS], $planet['b' . GID_B_NANITES], $speed));
        $row = [ '', $user['player_id'], $planet_id, $list_id, $id, $lvl, $destroy, $now, $now + $duration ];
        $sub_id = AddDBRow($row, 'buildqueue');
        if ($list_id == 1) {
            AddQueue($user['player_id'], $BuildEvent, $sub_id, $id, $lvl, $now, $duration, QUEUE_PRIO_BUILD);
        }
    }
...
function BuildDeque($user, $planet_id, $listid)
{
    ...
    if (dbrows($result)) {
        ...
        // Start the next construction
        if ($queue_id) {
            PropagateBuildQueue($planet_id, time());
        }
    }
...
function Queue_Build_End($queue)
{
    ...
    $effectiveEnd = max($lastPeek, (int) $queue['end']);
    if ($effectiveEnd > $lastPeek) {
        ProdResources($planet, $lastPeek, $effectiveEnd);
    }
    ...
    if ($queue['type'] === QTYP_BUILD) {
        ...
        if ($id == GID_B_TERRAFORMER) {
            $fields .= ', maxfields = maxfields + 5';
        }
        if ($id == GID_B_LUNAR_BASE) {
...
function AddShipyard($player_id, $planet_id, $gid, $value, $now = 0)
{
    ...
    if (in_array($gid, $defmap)) {
        UserLog($player_id, 'DEFENSE', va(loca_lang('DEBUG_LOG_DEFENSE', $GlobalUni['lang']), loca("NAME_$gid"), $value, $planet_id));
    } else {
        UserLog($player_id, 'SHIPYARD', va(loca_lang('DEBUG_LOG_SHIPYARD', $GlobalUni['lang']), loca("NAME_$gid"), $value, $planet_id));
    }
    ...
    if (($gid == GID_D_SDOME || $gid == GID_D_LDOME) && $planet['d' . $gid] > 0) {
        return;
    }
    ...
    while ($tasknum--) {
        ...
        if ($queue['obj_id'] == GID_D_SDOME || $queue['obj_id'] == GID_D_LDOME) {
            if ($queue['obj_id'] == $gid) {
                return;
            }    // is in line to build a dome of the same type.
        }
        if ($queue['obj_id'] == GID_D_ABM || $queue['obj_id'] == GID_D_IPM) {
            if ($queue['obj_id'] == GID_D_ABM) {
    ...
    if (IsEnoughResources($planet, $m, $k, $d, $e) && checkRequirements($gid, $user, $planet)['met']) {
        $speed = $uni['speed'];
...
function CanResearch($user, $planet, $id, $lvl)
{
...

    {
        ...
        $resq = dbarray($result);
        if ($resq) {
            return loca_lang('BUILD_ERROR_RESEARCH_ALREADY', $user['lang']);
        }
        ...
        if ($busy) {
            return loca_lang('BUILD_ERROR_RESEARCH_LAB_BUILDING', $user['lang']);
        }
        ...
        // You can't build in vacation mode
        elseif ($user['vacation']) {
            return loca_lang('BUILD_ERROR_RESEARCH_VACATION', $user['lang']);
        }

        // You can't research on foreign planet
        elseif ($planet['owner_id'] != $user['player_id']) {
            return loca_lang('BUILD_ERROR_INVALID_PLANET', $user['lang']);
        } elseif (!IsEnoughResources($planet, $m, $k, $d, $e)) {
            return loca_lang('BUILD_ERROR_NO_RES', $user['lang']);
        } elseif (!checkRequirements($id, $user, $planet)['met']) {
            return loca_lang('BUILD_ERROR_REQUIREMENTS', $user['lang']);
        }
...
function StopResearch($player_id)
{
    ...
    if ($result == null) {
        return;
    }        // The research is not in progress.
    $resq = dbarray($result);
    ...
    if ($planet['owner_id'] != $player_id) {
        Error(
            va(
                loca_lang('DEBUG_QUEUE_CANCEL_RESEARCH_FOREIGN', $GlobalUni['lang']),
                loca("NAME_$id"),
                $user['oname'],
...
function AddRecalcPointsEvent($player_id)
{
    ...
    if (dbrows($result) == 0) {
        ...
        $queue = [ null, $player_id, QTYP_RECALC_POINTS, 0, 0, 0, $now, $when, QUEUE_PRIO_RECALC_POINTS ];
...
function AddAllowNameEvent($player_id)
{
    ...
    if (dbrows($result) == 0) {
        $now = time();
        $when = $now + 7 * 24 * 60 * 60;
        $queue = [ null, $player_id, QTYP_ALLOW_NAME, 0, 0, 0, $now, $when, 0 ];
        $id = AddDBRow($queue, 'queue');
...
function AddChangeEmailEvent($player_id)
{
    ...
    $now = time();
    $when = $now + 7 * 24 * 60 * 60;
    $queue = [ null, $player_id, QTYP_CHANGE_EMAIL, 0, 0, 0, $now, $when, 0 ];
    $id = AddDBRow($queue, 'queue');
}
...
function AddUpdateStatsEvent($now = 0)
{
    ...
    if (dbrows($result) == 0) {
        ...
        if ($hours >= 8 && $hours < 16) {
            ...
            $when = mktime(8, 5, 0, $today['mon'], $today['mday'] + 1);
        }

        $queue = [ null, USER_SPACE, QTYP_UPDATE_STATS, 0, 0, 0, $now, $when, QUEUE_PRIO_UPDATE_STATS ];
        AddDBRow($queue, 'queue');
    }
}
...
function AddReloginEvent()
{
    ...
    if (dbrows($result) == 0) {
        ...
        ;
        $queue = [ null, USER_SPACE, QTYP_UNLOAD_ALL, 0, 0, 0, $now, $when, QUEUE_PRIO_RELOGIN ];
        $id = AddDBRow($queue, 'queue');
    }
}
...
function AddCleanDebrisEvent()
{
    ...
    if (dbrows($result) == 0) {
        ...
        $queue = [ null, USER_SPACE, QTYP_CLEAN_DEBRIS, 0, 0, 0, $now, $when, QUEUE_PRIO_CLEAN_DEBRIS ];
        $id = AddDBRow($queue, 'queue');
    }
}
...
function AddCleanPlanetsEvent()
{
    ...
    if (dbrows($result) == 0) {
        ...
        $queue = [ null, USER_SPACE, QTYP_CLEAN_PLANETS, 0, 0, 0, $now, $when, QUEUE_PRIO_CLEAN_PLANETS ];
        $id = AddDBRow($queue, 'queue');
    }
}
...
function AddCleanPlayersEvent()
{
    ...
    if (dbrows($result) == 0) {
        $now = time();
        $when = mktime(1, 10, 0, date('m'), date('d') + 1, date('y'));
        $queue = [ null, USER_SPACE, QTYP_CLEAN_PLAYERS, 0, 0, 0, $now, $when, QUEUE_PRIO_CLEAN_PLAYERS ];
        $id = AddDBRow($queue, 'queue');
    }
}
...
function AddRecalcAllyPointsEvent()
{
    ...
    if (dbrows($result) == 0) {
        $now = time();
        $when = mktime(0, 10, 0, date('m'), date('d') + 1, date('y'));
        $queue = [ null, USER_SPACE, QTYP_RECALC_ALLY_POINTS, 0, 0, 0, $now, $when, QUEUE_PRIO_RECALC_ALLY_POINTS ];
        AddDBRow($queue, 'queue');
    }
}
...
{
    $now = time();
    $queue = [ null, USER_SPACE, QTYP_DEBUG, 0, 0, 0, $now, $when, QUEUE_PRIO_DEBUG ];
    $id = AddDBRow($queue, 'queue');
}
// Debug Event.
...
function EnumOwnFleetQueue($player_id, $ipm = 0)
{
    global $db_prefix;
    if ($ipm) {
        $query = 'SELECT * FROM ' . $db_prefix . "queue WHERE type = '" . QTYP_FLEET . "' AND owner_id = $player_id ORDER BY end ASC, prio DESC";
    } else {
